---
name: peoplenometry
description: The complete Peoplenometry people-analytics toolkit in one skill. Analyze any workforce/HR data question in plain language — headcount and growth, tenure, attrition, comp and compa-ratio basics, diversity representation, span of control, promotions and internal mobility. It cleans messy exports, computes strictly from a canonical metrics library so numbers are defensible, answers in plain English with the methodology attached, learns your company's definitions as you work, and renders leadership-ready HTML/PDF reports. Use for "what's our headcount trend", "how long do people stay", "why are people leaving", "is our pay fair", "clean up this export", "turn this into a report", or any "what does our people data say about…".
---

# Peoplenometry

Ask a question about your people data in plain language. This one skill does the whole job — cleans the export if it's messy, runs the analysis with the same rigor a specialist would use, and renders a leadership-ready report. No data team required.
...
**This skill carries its own playbooks** in `references/`. Read the relevant one in full when the moment calls for it:
- `references/cleaning-data.md` — turn a messy HR export into an analysis-ready file (run first if the data is rough).
- `references/attrition-deep-dive.md` — the deep "why are people leaving" analysis: rates vs fair baselines, segment and tenure diagnostics, drivers, at-risk segments.
- `references/writing-reports.md` — render a finished analysis into a self-contained HTML report that prints to PDF.
...
1. Read `references/metrics-library.md`. This is the source of truth for every calculation.
...
## 1. Clean, go deep, or analyze — decide in one beat
Read the user's question and inspect the file. Then:
- **Messy data** (grain unclear, dates chaos, duplicates, rehires) → **follow `references/cleaning-data.md` first**, then come back. State specifically what's broken; if it's only mildly rough, say so and proceed with the issues flagged.
- **Deep attrition** (drivers, at-risk segments, tenure diagnostics, "why are people leaving") → **follow `references/attrition-deep-dive.md`** in full — it's the specialist method. A quick "what's our attrition rate?" does NOT need it; answer at a glance here, then offer the deep dive.
- **Rendering** ("make this presentable", "I need to send this up") → after the analysis, **follow `references/writing-reports.md`** to build the report.
- **Otherwise** → analyze right here (step 2 on).
...
- **Attrition at a glance** — headline rate per the library (leavers ÷ average headcount, annualized; transfers excluded; numerator and denominator shown). One-level segment split at most. Anything deeper → `references/attrition-deep-dive.md`.
...
Save to `people-analytics/analysis-<topic>-<YYYY-MM-DD>.md` with `result_format: 1` frontmatter (skill: peoplenometry; one `metrics:` entry per computed figure with value, numerator, denominator, n; house_rules_applied; caveats). This is what `references/writing-reports.md` renders — always save it.
...
### Close
Offer the natural next step, specifically: *"Want this as a leadership-ready document? I can render it with the methodology attached."* (follow `references/writing-reports.md`) — or the deep attrition dive, or the hottest follow-up question the data raised.
