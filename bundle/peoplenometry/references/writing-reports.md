...
## 0. Find the result
1. If invoked right after an analysis: read the saved artifact `people-analytics/analysis-*.md` from this session (written by `Peoplenometry`, `the attrition deep dive (attrition-deep-dive.md)`, or any analysis skill). **The file is the source of truth for every number**; conversation is context (tone, audience) only.
...
4. Read `.Peoplenometry/house-rules.md` if present (audience, official-number method, any report preferences). Echo what you're applying.
...
The visual system lives at `report-design-system.html`. It is the single source of truth for every report this skill emits.

Before generating any report:
1. **Read `report-design-system.html` in full.** Do not generate from memory of a previous run — the user may have edited it to rebrand.
...
- **Chart-interaction script:** in addition to the `:root` tokens and REPORT CSS, inline the `<script>` marked **REPORT JS** (bottom of `report-design-system.html`) into every report, just before `</body>`. Copy it verbatim; never link it.
