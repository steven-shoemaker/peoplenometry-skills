...
## 0. Load house rules
Read `.Peoplenometry/house-rules.md` (cwd), else `~/.people-analytics/house-rules.md`, if present. If a **data dictionary** entry for this source already exists (column mappings, headcount scope), apply and **echo** it — *"Using your saved mapping for the Workday export"* — and skip re-asking those questions. Also honor any **Local metrics** section — derived-field conventions defined there apply when building derived columns.
...
- **Derived fields:** tenure (per the metrics library (`metrics-library.md`), plus any local-metric conventions in house-rules), active/inactive flag.
...
Offer once at the end to save the confirmed **column mappings + headcount scope** to `house-rules.md` under `## Data dictionary` — the same file `Peoplenometry` reads for house decisions and local metrics, so every skill benefits — so no future run re-asks: show the exact lines, one `AskUserQuestion` — *Save all · Let me pick · Don't save*. Append-only; confirm first; on conflict with an existing rule, surface both and ask which wins.

### Close
*"Your data's ready. Want to analyze it? `Peoplenometry` for any question, or `the attrition deep dive (attrition-deep-dive.md)` to go deep on attrition. (Heads-up from the report: [biggest limitation].)"*
