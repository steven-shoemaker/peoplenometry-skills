---
name: why-are-people-leaving
description: Analyze employee attrition/turnover from an HR export — compute real attrition rates, find which teams/tenures/levels are losing people, surface likely drivers, and flag which segments carry elevated risk (segments, never named individuals). Produces a CHRO-ready narrative plus the methodology. Use for "why are people leaving", "our turnover is up", "attrition analysis", "who's quitting and why", "retention risk".
---
...
**Needs analysis-ready data.** If the export is messy, run `/clean-my-export` first — attrition math is very sensitive to bad dates, duplicate rows, and rehires.
